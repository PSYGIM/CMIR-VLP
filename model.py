...
class LXRTXLayer(BertPreTrainedModel):
    ...
    def forward(self, visn_feats, visn_attention_mask,
        ...
        # idx = idx.view(-1, 1)
        #
        # vl_output = self.itm_head(lang_feats[:,0,:])
        #

        lang_feats_c,visn_feats_c = self.cross_att(lang_feats, lang_attention_mask,visn_feats, visn_attention_mask)
        visn_feats = visn_feats + visn_feats_c
        lang_feats = lang_feats + lang_feats_c
        visn_feats = self.vision_proj(visn_feats)
        lang_feats = self.text_proj(lang_feats)
        # local alignment
        row_sums = lang_attention_mask.sum(axis=1)
        row_sums = [torch.tensor(x) for x in row_sums]
        # sims_g = image @ text.t()
        image,text = [], []
        loss = self.forward_loss(visn_feats, lang_feats,row_sums,image,text)
